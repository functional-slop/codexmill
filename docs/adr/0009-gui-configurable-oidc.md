...
> **Superseded in part by [ADR 0024](0024-local-accounts.md):** the "admin API is open while OIDC is
> unconfigured" bootstrap is closed (auth is mandatory), and secrets-at-rest are now ALWAYS encrypted
> (not file-perms-only). The config-store + GUI-OIDC decisions below still stand.
