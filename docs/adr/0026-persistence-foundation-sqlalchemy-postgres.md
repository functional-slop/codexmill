...
from the start, replacing the bespoke `ALTER TABLE` guards. A baseline migration creates the current
schema from the model metadata; every later schema change ships as a migration.

Do this now, not later: it is strictly cheaper while the schema is small and no instance holds data.
`create_all` never alters an existing table, so column additions to a live table need a migration
regardless; and retrofitting Alembic after instances exist means reverse-engineering the accumulated
schema and stamping deployed databases. Adopting it on the ground floor avoids both.
